...
dataset = load_dataset("json", data_files={
        "train": "./ted-multiling-filtered2/train.json",
        "test": "./ted-multiling-filtered2/test.json",
        "dev": "./ted-multiling-filtered2/dev.json"
    })

dataset = dataset.remove_columns(["pt", "calv", "fr-ca", "ku", "zh"])

checkpoint = "facebook/nllb-200-3.3B"
tokenizer = AutoTokenizer.from_pretrained(checkpoint)

def _tokenize_fn(row):
    tokenizer.src_lang = PART1_TO_FLORES["en"]
    tok_en = tokenizer(row["en"], return_tensors='pt', padding=False, truncation=False)["input_ids"]
    ...
    for k in list(row.keys()):
        ...
        tokenizer.src_lang = PART1_TO_FLORES[k]
        tok_hy = tokenizer(row[k], return_tensors='pt', padding=False, truncation=False)["input_ids"]
        ...
        # if l1/l2 > 1.5 or l1/l2 < 0.5:
        #     print(f"Removing: {row[k]}")
        #     print(f"Original: {row['en']}")
...
train_dataset = dataset["train"].map(_tokenize_fn, num_proc=10)
test_dataset = dataset["test"].map(_tokenize_fn, num_proc=10)
dev_dataset = dataset["dev"].map(_tokenize_fn, num_proc=10)

counts = [ (k, sum([ i != "__NULL__" for i in train_dataset[k] ])) for k in train_dataset.column_names ]
print("Counts before:")
[ print(i) if not i[0].startswith("ratio-") else None for i in counts ]
...
for k in train_dataset.column_names:
    ...
    vals = np.array(train_dataset["ratio-" + k])
    vals = vals[vals > 0]
    avg_ratios[k] = np.mean(vals)
    std_ratios[k] = np.std(vals)

    if not np.isfinite(avg_ratios[k]) or not np.isfinite(avg_ratios[k]):
        avg_ratios[k] = 1
        std_ratios[k] = 5

    print(f"{k}: 2 std removal rate: ", np.mean(np.abs(vals - avg_ratios[k]) > 2*std_ratios[k]) )
    print(f"{k}: 2.5 std removal rate: ", np.mean(np.abs(vals - avg_ratios[k]) > 2.5*std_ratios[k]) )
...
def func(row):
    ...
    for k in list(row.keys()):
        ...
        if avg > 0 and np.abs(ratio - avg) > 3*std:
            newRow[k] = "__NULL__"
        else:
            newRow[k] = row[k]
...
cols_to_remove = [k for k in train_dataset.column_names if k.startswith("ratio-")]
train_dataset2 = train_dataset.map(func, num_proc=10, remove_columns=cols_to_remove)
test_dataset2 = test_dataset.map(func, num_proc=10, remove_columns=cols_to_remove)
dev_dataset2 = dev_dataset.map(func, num_proc=10, remove_columns=cols_to_remove)

counts = [ (k, sum([ i != "__NULL__" for i in train_dataset2[k] ])) for k in train_dataset2.column_names ]
print("Counts after:")
[ print(i) for i in counts ]

train_dataset2.to_json("./ted-multiling-filtered3/train.json")
test_dataset2.to_json("./ted-multiling-filtered3/test.json")
dev_dataset2.to_json("./ted-multiling-filtered3/dev.json")